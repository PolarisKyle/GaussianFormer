...
class _LocalAggregate(torch.autograd.Function):
    ...
    def backward(ctx, logits_grad, bin_logits_grad, density_grad):
        ...
        if logits_grad.dim() != 2 or semantics.dim() != 2:
            raise RuntimeError(
                f'LocalAggregateProb backward expects 2D tensors, got logits_grad={tuple(logits_grad.shape)}, semantics={tuple(semantics.shape)}'
            )
        if logits_grad.shape[1] != semantics.shape[1]:
            raise RuntimeError(
                f'LocalAggregateProb channel mismatch before CUDA call: logits_grad C={logits_grad.shape[1]} vs semantics C={semantics.shape[1]}'
            )
